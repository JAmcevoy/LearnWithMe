...
const PostContent = () => {
  ...
  useEffect(() => {
    setLoading(true);
    fetchPosts("/posts/"); // Initial fetch
  }, []);
  ...
  return (
    <div className="flex flex-row flex-wrap justify-center gap-6 py-8 px-4 min-h-screen">
      <InfiniteScroll
        ...
        next={() => fetchPosts(posts.next)} // Fetch more data
        hasMore={!!posts.next}
        ...
        {posts.results.map((post) => (
          ...
          >
            <div className="flex items-center p-4 border-b border-gray-200">
              <img
                src={post.owner.image|| "default-profile-pic.jpg"}
                alt={`${post.owner.username}'s profile`}
                className="w-12 h-12 rounded-full object-cover"
              />
              <div className="ml-4">
                <h2 className="font-semibold">{post.owner.username}</h2> 
                <p className="text-gray-500 text-sm">{new Date(post.created_at).toLocaleDateString()}</p>
              </div>
            </div>
            <img
              src={post.image_or_video || "default-image.jpg"} // Handle missing image/video
              alt={post.title}
              className="w-full h-auto object-cover"
            />
            <div className="p-4">
              <p className="text-gray-700 mb-4">{post.title}</p> {/* Adjust as necessary */}
